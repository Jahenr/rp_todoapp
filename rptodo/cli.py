...
#from rptodo.database import DEFAULT_DB_FILE_PATH
...
def list_all() -> None:
    ...
    if len(todo_list) == 0:
        typer.secho(
            "There are no tasks in the to-do list yet", fg=typer.colors.GREEN
        )
        raise typer.Exit()
...
def remove(
    ...
    if force:
        ...
        if delete:
            ...
            typer.echo("Operation canceld")

@app.command(name="clear")
def remove_all(
    force: bool = typer.Option(
        ...,
        prompt="Delete all to-do's?",
        help="Force deletion of all tasks without confirmation.",
    ),
) -> None:
    """Remove all to-dos."""
    todoer = get_todoer()
    if force:
        ...
        if error:
            typer.secho(
                f'Removing the to-dos failed with "{ERRORS[error]}"',
                fg=typer.colors.RED,
            )
            raise typer.Exit(1)
        else:
